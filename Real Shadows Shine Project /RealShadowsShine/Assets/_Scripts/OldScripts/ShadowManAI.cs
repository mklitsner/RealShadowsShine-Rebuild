...
public class ShadowManAI : MonoBehaviour {
	...
	void Update () {
		...
		float wandererDistance=50;
		if (GameObject.FindGameObjectWithTag("wanderer") != null) {
			wanderer = GameObject.FindGameObjectWithTag("wanderer");
			wandererDistance = Vector3.Distance (wanderer.transform.position, transform.position);
		...
		if (wandererDistance < 5) {
			...
			if (RandomSign()==1 && GameObject.FindGameObjectWithTag("wanderer") != null) {
				hasDirection = true;
				currentDirection = Quaternion.LookRotation (wanderer.transform.position - transform.position).eulerAngles;
			} else {
				hasDirection = false;
